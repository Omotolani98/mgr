...
- `inventory.yaml` — `inventory.FileStore`, a versioned list of `Server` records. Keyed by `Name` **or** `ID`; `Upsert` dedups on either. IDs are slugified from the name, tags are lowercased/deduped/sorted, servers are sorted by name on save. Missing file = empty inventory (not an error).
